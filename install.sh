...
systemctl enable sysctl-custom
    ...
    if [ "$gost_version_choice" -eq 1 ]; then
        ...
        wget https://github.com/ABOLFAZLnova/gost/releases/download/v2.11.5/gost-linux-amd64-2.11.5.gz && \
    ...
    wget -O /tmp/gost.tar.gz https://github.com/ABOLFAZLnova/gost2/releases/download/v3.0.0-nightly.20240426/gost_3.0.0-nightly.20240426_linux_amd64.tar.gz
...
# If option 6 is selected
    ...
    case "$gost_version_choice" in
        ...
        2)
            ...
            latest_version=$(curl -s https://api.github.com/repos/ABOLFAZLnova/gost/releases/latest | grep 'tag_name' | cut -d\" -f4) && \
            wget -O /tmp/gost.tar.gz "https://github.com/ABOLFAZLnova/gost/releases/download/$latest_version/gost-linux-amd64-$latest_version.tar.gz" && \
            tar -xvzf /tmp/gost.tar.gz -C /usr/local/bin/ && \
            chmod +x /usr/local/bin/gost && \
            echo $'\e[32mGost installed successfully.\e[0m'
            ;;
...
if [ "$choice" -eq 9 ]; then
    ...
    wget -N --no-check-certificate https://github.com/ABOLFAZLnova/across/raw/master/bbr.sh && \
    chmod +x bbr.sh && \
    bash bbr.sh
    bash "$0"
